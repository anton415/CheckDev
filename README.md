...
Клонируйте проект вместе с сервисами:

```bash
git clone --recurse-submodules https://github.com/<your-account>/CheckDev.git
```

Если проект уже скачан без сервисов, выполните:
...
Из корня проекта можно поднять полный локальный стек CheckDev в контейнерах:
...
- `db` на базе образа `postgres:16` c отдельными базами для сервисов;
- `kafka` для событий `site` и `notification`;
- `eureka`, `auth`, `desc`, `mock`, `generator`, `notification`, `site`,
  которые собираются из корневого `Dockerfile` через multi-stage build.
...
Полный сброс контейнеров вместе с volume PostgreSQL:
...
После обновления главного репозитория обязательно подтяните версии сервисов:
...
Код каждого сервиса хранится в отдельном репозитории и подключается в CheckDev как submodule.
Если задача затрагивает сервис в каталоге `services/*`, изменения нужно закоммитить и отправить
в соответствующий репозиторий сервиса, а затем в репозитории CheckDev закоммитить только
обновлённый указатель submodule на новый commit сервиса.
